...
.. figure:: ../figs/boolode-example.png
   ...
   If there is an equal probability of taking either the left hand branch
...
.. note:: The initial condition file should also be tab
           separated. Also, pay attention to the capitalisation
           of the column names.
...
.. code:: text
          ...
          |-- GroundTruthNetwork.csv         # Boolean network represented as an edge list
          `-- simulations/
              |-- E1.csv
              |-- E2.csv
              ...
...
In order to visualise the entire dataset, we can carry out dimensionality reduction using
...
is used to colour each cell in the scatter plot. Darker colours imply early time points in the
simulation. The output should look like the following

.. figure:: ../figs/tree.png
   :align: center

   t-SNE visualisation of BoolODE output
