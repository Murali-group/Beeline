...
def main(config_map, opts):
    ...
    if (opts.auc):
        ...
        if opts.tfs:
            print('\n\nConsidering only TF-gene interactions...')
        AUPRC, AUROC, trueEdgeCnt, possEdgeCnt, TFCnt, nodeCnt = evalSummarizer.computeAUC(TFEdges = opts.tfs)
        
        AUPRC.to_csv(outDir+'AUPRC.csv')
        AUROC.to_csv(outDir+'AUROC.csv')
        
        trueEdgeCnt.to_csv(outDir+'trueEC.csv')
        possEdgeCnt.to_csv(outDir+'possibleEC.csv')
        
        TFCnt.to_csv(outDir+'TFCnt.csv')
        nodeCnt.to_csv(outDir+'nodeCnt.csv')
    ...
    if (opts.epr):
        ...
        ePRDF = evalSummarizer.computeEarlyPrec(TFEdges = opts.tfs)
