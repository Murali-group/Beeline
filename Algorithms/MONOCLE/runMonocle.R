...
# for now, just use all genes
#ordering_genes <- row.names(geneData)
...
# also compute the differential expression varying across time
diff_test_res <- differentialGeneTest(CDS, fullModelFormulaStr="~sm.ns(Pseudotime)", cores=30)
#diff_test_res[,c("gene_short_name", "pval", "qval")]
write.csv(diff_test_res, file= paste0(arguments$outPrefix,'DiffExp.csv'), quote = FALSE)
}
